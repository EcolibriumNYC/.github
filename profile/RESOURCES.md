# Learning & Development Resources

Guides, required runtimes, and reference material for contributing across our hardware and software repositories.

## Tooling & Environment
* We standardize toolchains and dependencies via [mise](https://mise.jdx.dev/).
* Follow the [mise Getting Started Guide](https://mise.jdx.dev/getting-started.html) to install.
* Run `mise install` in any project root to provision the required runtimes.

## Collaboration & Version Control
* We collaborate via GitHub pull requests. Review [The Odin Project: Git Basics](https://www.theodinproject.com/lessons/foundations-git-basics) if you need a workflow refresher.

## Foundations
* **Command Line:** [MDN Command Line Guide](https://developer.mozilla.org/en-US/docs/Learn/Tools_and_testing/Understanding_client-side_tools/Command_line)
* **Web Architecture:** [How the Internet Works](https://developer.mozilla.org/en-US/docs/Learn/Common_questions/Web_mechanics/How_does_the_Internet_work) · [How the Web Works](https://developer.mozilla.org/en-US/docs/Learn/Getting_started_with_the_web/How_the_Web_works)
* **Code Editors:** [MDN Editor Basics](https://developer.mozilla.org/en-US/docs/Learn/Getting_started_with_the_web/Installing_basic_software)

## Hardware & Embedded Systems
* [UTexas: Intro to Embedded Systems](https://users.ece.utexas.edu/~valvano/Volume1/)
* [Instructables: Soldering Guide](https://www.instructables.com/How-to-solder/)

## Analysis & Diagnostics
* **Network Inspection:** [freeCodeCamp: Wireshark Guide](https://www.freecodecamp.org/news/how-to-use-wireshark/)
* **Binary Analysis:** [Ghidra Beginner Guide](https://ghidra-sre.org/)
